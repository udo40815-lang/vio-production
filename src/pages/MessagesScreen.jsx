...
// Conversation list with unread badges, last message previews, and profiles.
// ============================================================================

import React, { useState, useEffect } from 'react';
import { MessagesSquare, Loader2, Clock, AlertCircle } from 'lucide-react';
import { V, timeAgo } from '../utils/design-system.js';
import { getUserConversations } from '../lib/messages.js';
...
function MessagesScreen({ ui, onOpenConversation }) {
  ...
  if (loading) {
    return (
      <section className="px-4 pt-6 pb-20">
        <h1 className="text-[24px] font-bold tracking-[-0.03em] mb-6" style={{ color: ui.textPrimary }}>
          Messages
        </h1>
    ...
    return (
      <section className="px-4 pt-6 pb-20">
        ...
        <div className="rounded-3xl p-10 text-center" style={{ background: ui.dark ? V.surfaceDark : '#FFFFFF', border: `1px solid ${ui.border}` }}>
          ...
          <div className="mt-1.5 text-[13px] leading-relaxed" style={{ color: ui.textSecondary }}>
            {error?.message || 'Something went wrong.'}
          </div>
          <button
            onClick={loadConversations}
            className="mt-4 h-[34px] px-5 rounded-full text-[13px] font-semibold transition-all duration-200 hover:scale-[1.02] active:scale-95"
  ...
  return (
    <section className="px-4 pt-6 pb-20">
      <h1 className="text-[24px] font-bold tracking-[-0.03em] mb-5" style={{ color: ui.textPrimary }}>
        Messages
      </h1>
      <div className="space-y-1">
        {conversations.map((conv) => (
          ...
          >
            {/* Avatar */}
            <Avatar
              handle={conv.other_user?.username || '?'}
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex items-center justify-between">
                ...
                >
                  {conv.last_message_preview || (
                    <span className="italic" style={{ color: ui.textMuted }}>No messages yet</span>
                  )}
                </span>
                {conv.unread_count > 0 && (
                  <span
                    className="shrink-0 ml-2 min-w-[20px] h-[20px] rounded-full flex items-center justify-center text-[10px] font-bold"
                    style={{ background: V.royal, color: '#FFF' }}
                  >
              ...
              <div className="text-[11px] mt-0.5" style={{ color: ui.textMuted }}>
