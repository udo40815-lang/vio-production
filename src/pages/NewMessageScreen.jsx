  ...
  return (
    <section className="px-4 pt-3 pb-20">
      ...
      <div className="flex items-center gap-3 mb-4">
        <button onClick={onBack} className="p-1 -ml-1 rounded-full" aria-label="Back">
          <ArrowLeft size={20} style={{ color: ui.textPrimary }} />
        </button>
        <h1 className="text-[20px] font-bold tracking-[-0.03em]" style={{ color: ui.textPrimary }}>
          New message
        </h1>
      </div>
      ...
      <div className="relative mb-4">
        ...
        <input
          ...
          autoFocus
          className="w-full h-[42px] pl-10 pr-4 rounded-xl text-[14px] outline-none transition-colors"
          style={{
      ...
      {error && !loading && (
        <div className="rounded-2xl p-6 text-center" style={{ background: ui.dark ? V.surfaceDark : '#FFFFFF', border: `1px solid ${ui.border}` }}>
          <div className="text-[14px]" style={{ color: ui.textSecondary }}>Something went wrong. Try again.</div>
        </div>
      )}
      ...
      {!loading && searched && !error && results.length === 0 && (
        <div className="rounded-2xl p-8 text-center" style={{ background: ui.dark ? V.surfaceDark : '#FFFFFF', border: `1px solid ${ui.border}` }}>
          <div className="mx-auto w-10 h-10 rounded-xl flex items-center justify-center mb-3" style={{ background: `${V.royal}12` }}>
            <Users size={18} style={{ color: ui.textMuted }} />
          </div>
          ...
          {results.map((profile) => (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <div className="text-[14px] font-semibold truncate leading-snug" style={{ color: ui.textPrimary }}>
