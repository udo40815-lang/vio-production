// ============================================================================
// Vio — ChatScreen (DM experience — text messaging)
// ============================================================================
...
function ChatScreen({ ui, conversation, onBack, onViewProfile }) {
  ...
  const loadOlder = useCallback(async () => {
    ...
    const { messages: older, error: e } = await getConversationMessages(conversation.id, { limit: PAGE_SIZE, offset });
    if (!e && older.length > 0) {
  ...
  const handleRetry = (msg) => { setMessages(prev => prev.filter(m => m.id !== msg.id)); setInput(msg.content || ''); inputRef.current?.focus(); };
      ...
      <div key={msg.id} className={`flex ${isMine ? 'justify-end' : 'justify-start'} mb-1.5 px-3`}>
        <div className={`flex items-end gap-2 max-w-[82%] ${isMine ? 'flex-row-reverse' : ''}`}>
          ...
          <div className="group relative">
            <div className={`px-3.5 py-2.5 text-[14px] leading-relaxed break-words ${isFailed ? 'opacity-60' : ''} ${isPending ? 'opacity-75' : ''}`}
            ...
            {showTs && !isPending && (
              <div className={`flex items-center gap-1.5 mt-0.5 px-1 ${isMine ? 'justify-end' : 'justify-start'}`}>
                {isFailed ? (
                  <button onClick={() => handleRetry(msg)} className="flex items-center gap-1" style={{ color: V.red }}><AlertCircle size={10} /><span className="text-[10px] font-medium">Failed — tap to retry</span></button>
                ) : (
                ...
                {isMine && !isDeleted && !isFailed && !isPending && (
                  <button onClick={() => handleDelete(msg.id)} disabled={isDeleting} className="opacity-0 group-hover:opacity-100 transition-opacity p-0.5">
            ...
            {isPending && (
              <div className={`flex items-center gap-1 mt-0.5 px-1 ${isMine ? 'justify-end' : 'justify-start'}`}>
                ...
                <span className="text-[10px]" style={{ color: isMine ? 'rgba(255,255,255,0.5)' : ui.textMuted }}>Sending...</span>
              </div>
            )}
          </div>
        </div>
  ...
  return (
    <section className="flex flex-col" style={{ height: '100vh', maxWidth: '600px', margin: '0 auto' }}>
      <div className="shrink-0 flex items-center gap-3 px-3 py-3 z-20" style={{ background: ui.dark ? 'rgba(9,9,11,0.94)' : 'rgba(248,248,250,0.94)', backdropFilter: 'blur(24px)', WebkitBackdropFilter: 'blur(24px)', borderBottom: `1px solid ${ui.border}` }}>
        <button onClick={onBack} className="p-1.5 rounded-full" aria-label="Back"><ArrowLeft size={18} style={{ color: ui.textPrimary }} /></button>
        <button onClick={() => { if (onViewProfile && otherUser?.user_id) { onBack(); setTimeout(() => onViewProfile(otherUser.user_id), 100); } }} className="flex items-center gap-3 flex-1 min-w-0" style={{ background: 'none', border: 'none', cursor: onViewProfile ? 'pointer' : 'default' }}>
          <Avatar handle={otherUser?.username} name={otherUser?.display_name} src={otherUser?.avatar_url || null} size={36} />
          <div className="flex-1 min-w-0 text-left"><div className="text-[14.5px] font-semibold truncate" style={{ color: ui.textPrimary }}>{otherUser?.display_name || otherUser?.username || 'Unknown'}</div><div className="text-[11px]" style={{ color: ui.textMuted }}>@{otherUser?.username || 'unknown'}</div></div>
        </button>
      </div>
      <div ref={msgListRef} className="flex-1 overflow-y-auto py-2" style={{ background: ui.bg }} onScroll={handleScroll}>
        ...
        {error && !loading && messages.length === 0 && (<div className="flex flex-col items-center justify-center py-12 px-4"><div className="text-[14px] mb-3" style={{ color: ui.textSecondary }}>{error?.message || 'Could not load messages.'}</div><button onClick={loadMessages} className="h-[30px] px-4 rounded-full text-[12px] font-semibold" style={{ background: V.royal, color: '#FFF' }}>Retry</button></div>)}
        {!loading && !error && messages.length === 0 && (<div className="flex flex-col items-center justify-center py-16 px-4"><div className="text-[14px]" style={{ color: ui.textMuted }}>Start the conversation</div><div className="text-[12px] mt-1" style={{ color: ui.textMuted }}>Send a message to begin chatting.</div></div>)}
      ...
      <div className="shrink-0 px-3 py-3" style={{ background: ui.dark ? '#09090B' : '#F8F8FA', borderTop: `1px solid ${ui.border}`, paddingBottom: 'max(12px, env(safe-area-inset-bottom))' }}>
        <div className="flex items-end gap-2">
          <div className="flex-1 rounded-2xl px-4 py-2.5" style={{ background: ui.dark ? V.surfaceDark : '#EBEBF0' }}>
            <textarea ref={inputRef} value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={handleKeyDown} placeholder="Write a message..." rows={1} disabled={sending} className="w-full bg-transparent text-[14px] leading-relaxed resize-none outline-none" style={{ color: ui.textPrimary, maxHeight: '80px', fontFamily: 'inherit' }} />
          </div>
          <button onClick={handleSend} disabled={!input.trim() || sending} className="shrink-0 w-[38px] h-[38px] rounded-full flex items-center justify-center transition-all duration-200" style={{ background: input.trim() && !sending ? V.royal : (ui.dark ? V.surfaceDark : '#D4D4DC'), opacity: input.trim() && !sending ? 1 : 0.5 }} aria-label="Send">
